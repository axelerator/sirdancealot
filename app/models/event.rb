class Event < Group
  ...
  validates :group, :place, :starts_at, :ends_at, presence: true
  ...
  def add_host!(institution)
    Relationships::HostedBy.create!(group: self, other_group: institution)
  ...
  def invite!(invitee)
    ...
    invitees.each do |i|
      Relationships::InvitedTo.create!(group: self, user: i)
    end
  end
